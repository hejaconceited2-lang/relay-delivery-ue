...
<div class="status-bar" id="statusBar">
  ...
  <div class="stat"><div class="dot" style="background:var(--g)"></div> 低难度 <strong id="cntGo" style="color:var(--g)">—</strong></div>
  <div class="stat"><div class="dot" style="background:var(--y)"></div> 中高难度 <strong id="cntMaybe" style="color:var(--y)">—</strong></div>
...
<div style="margin-top:20px">
  ...
  <div class="tips" id="tips">
    <h3>🗓 从评估到开点</h3>
    <div class="tip-grid">
      <div class="tip-item"><span class="ico">🔍</span><span><strong>① 踩点评估</strong><br><small>外卖柜（有一票否决）→ 骑手路线/物业态度/摆放空间（决定难度高低，都要做）</small></span></div>
...
<script type="module">
  ...
  const BATCH1 = [
    ["p01", "珠江国际纺织城",     "海珠区", "凤阳街道",  "商场/购物广场", 1039.6, null],
    ["p02", "中大附属第六医院",    "天河区", "员村街道",  "医院",          450,    null],
    ["p03", "中大附属第三医院",    "天河区", "石牌街道",  "医院",          640,    null],
    ["p04", "中大附三岭南医院",    "黄埔区", "联和街道",  "医院",          234.4,  null],
    ["p05", "云升科学园",         "黄埔区", "联和街道",  "写字楼/园区",   439.8,  null],
  ...
  function assess(scouting) {
    ...
    if (scouting.shelter === "有遮挡") good++;
    const filled = countFilled(scouting);
    if (filled < 4) return { level: "gray", text: "待完善", cls: "gray", detail: `已填${filled}/4项，还差${4-filled}项` };
    if (good >= 2) return { level: "green", text: "🟢 低难度", cls: "green", detail: `${good}/3项有利，条件较好` };
    if (good === 1) return { level: "yellow", text: "🟡 中等难度", cls: "yellow", detail: "仅1项有利，运营需多投入" };
  ...
  window.editName = async function(id, currentName) {
    const name = prompt('修改认领人姓名：', currentName);
    if (!name || !name.trim() || name.trim() === currentName) return;
    try {
      await set(ref(db, `points/${id}/name`), name.trim());
    } catch (e) {
      alert("修改失败: " + e.message + "\n\n请确认 Firebase 安全规则已更新（name 字段允许写入已认领点位）");
    }
  };
